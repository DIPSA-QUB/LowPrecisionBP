//
// Created by Mark Van der Merwe, Fall 2018, Edited by Amir, March 2021
//
...
//-----------------------------------------
std::tuple<float, std::vector<float>, int, std::vector<std::pair<int, int>>, std::vector<std::pair<float, int>>> infer(pgm* pgm, double epsilon, int timeout, std::vector<int> runtime_params, bool verbose) {
  
  int num_edges = pgm->edge_idx_to_edges_idx.size();
  int edge_size = pgm->edges_half.size();
  ...
  workspace.resize(pgm->edges_half.size());
  ...
  std::cout <<"\n Iterations = "<<iterations<< "Time: " << converge_time << " ms." << std::endl;
  //rpq.print();
