...
int main(int argc, char *argv[]) {
  ...
  std::tuple<float, std::vector<float>, int, std::vector<std::pair<int, int>>, std::vector<std::pair<float,int>>> results = infer(test_pgm, 0.2, timeout, runtime_params, true);
  
  std::cout << "\n Runtime: " << std::get<0>(results) << " ms." << std::endl;

  //print_array(std::get<1>(results)); 
  
  //---------------- write marginals to file
  std::ofstream marginals_file1;
  marginals_file1.open("1_Ising_17_c_2_e_0_2_mini_3_5_marginals.txt");
  
  
  marginals_file1 << "1_Ising_17_c_2_e_0.2_mini_3_5_marginals \n" << "Runtime: " << std::get<0>(results) << "\n"<< "Iterations: " << std::get<2>(results) << std::endl;
 
  marginals_file1 << test_pgm->marginal_rep.size() << "\n";
  
  for (float marginal: test_pgm->marginal_rep) {
