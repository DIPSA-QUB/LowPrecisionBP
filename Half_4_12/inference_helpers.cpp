//
// Created by Mark Van der Merwe, Fall 2018, data type changed by Amir Sabbagh Molahosseini, 2022
//
...
uint16_t encode_to_half(float p)
{
...
//-------------------------------------------------  
float decode_to_float(uint16_t p)
{
   union uint32_float num={0};
   num.half_value = p;
   num.uint_value = ( num.uint_value << 11 );
   num.uint_value = (num.uint_value | MASK_EXP1_4);
   return(num.value); 
}
//------------------------------------------------- 
...
//-------------------------------------------- 
double message_delta(std::vector<uint16_t> edges_half, std::vector<float> workspace, int edge_start) 
{
