...
class pgm {
 ...
 public:
  ...
  //std::vector<float> edges; // Store the representations for the values.
  std::vector<uint16_t> edges_half;  // Store the representations for the values with proposed half precision.
  
  // Represent the factors on edges.
  std::vector<int> edge_idx_to_edge_factors_idx; // Get the edge factor for the given edge.
  std::vector<float> edge_factors; // Store the representations for the edge factors.

  // Represent the node factors.
  std::vector<int> edge_idx_to_node_factors_idx; // Get the node factor for the given edge.
  std::vector<float> node_factors; // Store the representations for the node factors.
  ...
  std::vector<float> marginal_rep; // Final belief representations.
