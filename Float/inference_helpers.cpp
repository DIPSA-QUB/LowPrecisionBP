...
float compute_message(pgm* pgm, std::vector<float> &workspace, int edge_id, bool write_to_edges) {
  ...
  float delta = message_delta(pgm->edges, workspace, edges_index);
...
//std::cout<<" message_length : "<<workspace[edge_start]<<"\n";
  float sqr_diff_sum = 0.0;
  for (int category = 0; category < message_length; ++category) {
     sqr_diff_sum += fabs(workspace[edge_start + 1 + category] - edges[edge_start + 1 + category]);
  }
  // std::cout<<" message delta: "<<sqr_diff_sum<<"\n";
