...
std::tuple<float, std::vector<float>, int, std::vector<std::pair<int, int>>, std::vector<std::pair<float, int>>> infer(pgm* pgm, float epsilon, int timeout, std::vector<int> runtime_params, bool verbose) {
  ...
  boost::heap::fibonacci_heap<std::pair<int, float>, boost::heap::compare<residual_pair_ordering>> rpq;
  typedef typename boost::heap::fibonacci_heap<std::pair<int, float>, boost::heap::compare<residual_pair_ordering>>::handle_type handle_t;
  std::vector<handle_t> edge_handles;
  for (int i = 0; i < num_edges; ++i) {
    handle_t handle_i = rpq.push(std::pair<int, float>(i, 10.0));
  ...
  while(!converged && time < timeout) {
    ...
    rpq.pop();
    float message_diff = compute_message(pgm, workspace, top_residual_element_id, true);
    edge_handles[top_residual_element_id] = rpq.push(std::pair<int, float>(top_residual_element_id, 0.0));
    ...
    for (int outgoing_edge_idx = outgoing_edge_start; outgoing_edge_idx < outgoing_edge_end; ++outgoing_edge_idx) {
      ...
      float outgoing_edge_residual = compute_message(pgm, workspace, outgoing_edge_id, false);
      rpq.update(edge_handles[outgoing_edge_id], std::pair<int, float>(outgoing_edge_id, outgoing_edge_residual));
    }

    converged = std::get<1>(rpq.top()) < epsilon;
    //std::cout<<iterations<<":   conv: "<<std::get<1>(rpq.top()) <<" epsilon: "<<epsilon<<" t/f: "<<converged<<"\n";
    since = std::clock();
