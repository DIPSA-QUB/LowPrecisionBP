...
//-------------------------------------------------- exp=2 bits    mantissa=14 bits   (3 should be reduced from float exponent before conversion)
uint16_t encode_to_half1(float p)
{
   union uint32_float1 num; 
   num.value = p;
   num.uint_value = num.uint_value - 25165824;
   num.uint_value = (num.uint_value>>9); 
   return(num.half_value);
}
//--------------------------------------------------
pgm::pgm(std::string input_file) {
  ...
  while (input != "") {
    ...
    for(std::string value: string_factor_split) {
      node_factors.push_back(std::stof(value));
    }
  ...
  while(input != "") {
    ...
    for(std::string value: string_factor_split) {
      edge_factors.push_back(std::stof(value));
    }

    // edge_idx_edges_idx + edges
    // Add edge encoding.
    // First edge: i->j - |j| categories.
    edge_idx_to_edges_idx.push_back(edges_half.size());
    edges_half.push_back(node2_categories);
    for (int i = 0; i < node2_categories; ++i) {
      edges_half.push_back(encode_to_half1((1.0/(float) node2_categories)));
    }

    // Second edge: j->i - |i| categories.
    edge_idx_to_edges_idx.push_back(edges_half.size());
    edges_half.push_back(node1_categories);
    for (int i = 0; i < node1_categories; ++i) {
      edges_half.push_back(encode_to_half1((1.0/(float) node1_categories)));
    }
...
void pgm::print() {
  ...
  //print_out_data(edges_half, "Edges: ");
